...
export const login = (usr:UserModel, history, checked:boolean): AppThunk => async dispatch => {
    try {
        ...
        const questions = await AuthService.login(usr)
        dispatch(LOG_g("data", questions.data.toString()))

        let user:UserModel=getResponseData(questions)
        dispatch(loginSuccess(user))
        axios.defaults.headers.common['Authorization'] = questions.data.token;
        if (checked === true) {
            saveTokenToLocalStorage(questions.data.token, user);
        }
